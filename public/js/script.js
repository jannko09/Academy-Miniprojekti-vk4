...
$("#addNewTaskButton").on("click", function () {
  console.log('TULEEKO PYYNTÖ2 CLIENTISTÄ?');
...
//Button opening popup window for new project
$("#defineNewProjectButton").on("click", function () {
  console.log('DEFINEPROJECT');
  $("#popup2").popup("show");

});

//Button sending new project information to server

$("#addNewProjectButton").on("click", function () {

  let projectName = $("#projectName").val();
  let ProjectDescription = $("#projectDes").val();
  let newProjectinfo = { "name": projectName, "date": etarg[0], "description": ProjectDescription };
  newProjectinfo = JSON.stringify(newProjectinfo);
  ...
  var settings = {
    "async": true,
    "crossDomain": true,
    "url": "http://localhost:3000/api/newproject",
    "method": "POST",
    "headers": {
      "Content-Type": "application/json",
      "cache-control": "no-cache",
